<template>
    <div class="container my-30" style="min-height: 820px;" :style="mode==='dark'?'background: #080035; color: #fff':''">
        <section class="hk-sec-wrapper">
            <h3 class="hk-sec-title text-center" :class="mode==='dark'?'text-primary':''">{{ $t('administrador.componentes.gamificacion.lista_acciones.titulo') }}</h3>
            ...
            <div v-if="!isTrashList" class="row justify-content-between px-10">
                <div class="col-sm">
                    <router-link class="btn btn-sm btn-labeled btn-danger float-left" :to="{ path: '/admin/gamification/actions', query: { 'only_trashed': 1 } }">
                        <span class="btn-label ml-1"><i class="fas fa-trash"></i></span>{{ $t('papelera') }}
                    </router-link>
                </div>
                <div class="col-sm">
                    <router-link class="btn btn-sm btn-labeled btn-success float-right" :to="{ path: '/admin/gamification/action' }">
                    ...
                    <table class="table table-sm table-hover table-bordered vld-parent" ref="tableContainer">
                        ...
                        <tr v-if="totalRows === 0">
                            <td :colspan="data.columns.length" class="text-center" style="height: 200px;">
                                <p v-if="!loadActions && totalRows === 0">
                                    {{ $t('administrador.table_list.no_entries') }}
                                </p>
                            </td>
                        </tr>
                        <tr v-for="action in data.rows" :key="action.id">
                            <td v-for="column in data.columns" :key="'action-' + action.id + '-' + column.field">
                                <p v-if="!column.customizable">{{ action[column.field] }}</p>
                                <div v-else-if="column.customizable && column.field === 'type'" class="text-center">
                                    <div class="badge badge-warning">{{ action.type }}</div>
                                </div>
                                <div v-else-if="column.customizable && column.field === 'subtype'" class="text-center">
                                    <div class="badge badge-info">{{ action.subtype }}</div>
                                </div>
                                <div v-else-if="column.field === 'actions'" class="text-center">
                                    ...
                                    <div class="dropdown-menu">
                                        <router-link v-if="!isTrashList" class="dropdown-item" :to="{ path: '/admin/gamification/action/' + action.id }">{{ $t('editar') }}</router-link>
                                        <button v-if="!isTrashList" @click="showDeleteActionModal(action.id)" class="dropdown-item">{{ $t('eliminar') }}</button>
                                        <button v-if="isTrashList" @click="showForceDeleteActionModal(action.id)" class="dropdown-item">{{ $t('eliminar_permanente') }}</button>
                                        <button v-if="isTrashList" @click="showUndeleteActionModal(action.id)" class="dropdown-item">{{ $t('restaurar') }}</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    <div class="row justify-content-end">
                        <ul v-if="totalRows > 0" class="pagination" style="overflow: auto;">
                            ...
                            <li class="page-item" v-bind:class="currentPage - 1 < 1 ? 'disabled' : ''">
                                <a
                                        class="page-link"
                                        @click="getActions(currentPage - 1)"
                            ...
                            <li
                                    ...
                                    class="page-item"
                            >
                                <a class="page-link" @click="getActions(i)">{{ i }}</a>
                            </li>
                            ...
                            <li class="page-item" v-bind:class="currentPage + 1 > Math.ceil(totalRows/limit) ? 'disabled' : ''">
                                <a
                                        class="page-link"
                                        @click="getActions(Math.ceil(totalRows/limit))"
            ...
            >
                <template v-slot:modal-header>
                    <h6 class="text-white">{{ $t('administrador.componentes.gamificacion.eliminar_accion.modal_eliminacion.titulo') }}</h6>
                </template>
                <div class="form-row">
                    <div class="col-md-12 mb-10">
                        <p>{{ $t('administrador.componentes.gamificacion.eliminar_accion.modal_eliminacion.pregunta') }}</p>
                    </div>
                </div>
                <template v-slot:modal-footer>
                    <div class="btn-group w-100">
                        <b-button
                                ...
                                variant="danger"
                                size="sm"
                                @click.prevent="deleteAction(false)"
                        >
                        ...
                        <b-button
                                variant="secondary"
                                size="sm"
                                @click.prevent="$bvModal.hide('delete-action-modal')"
                        >
            ...
            >
                <template v-slot:modal-header>
                    <h6 class="text-white">{{ $t('administrador.componentes.gamificacion.eliminar_accion.modal_forzar_eliminacion.titulo') }}</h6>
                </template>
                <div class="form-row">
                    <div class="col-md-12 mb-10">
                        <p>{{ $t('administrador.componentes.gamificacion.eliminar_accion.modal_forzar_eliminacion.pregunta') }}</p>
                ...
                <template v-slot:modal-footer>
                    <div class="btn-group w-100">
                        <b-button
                                ...
                                variant="danger"
                                size="sm"
                                @click.prevent="deleteAction(true)"
                        >
                            <i class="fas fa-trash"></i> {{ $t('si') }}
                            <Loading
                                    :active.sync="loadModalBtn"
                        ...
                        <b-button
                                variant="secondary"
                                size="sm"
                                @click.prevent="$bvModal.hide('force-delete-action-modal')"
                        >
                            <i class="fas fa-window-close"></i> {{ $t('no') }}
                        </b-button>
                    </div>
            ...
            >
                <template v-slot:modal-header>
                    <h6 class="text-white">{{ $t('administrador.componentes.gamificacion.eliminar_accion.modal_restaurar.titulo') }}</h6>
                </template>
                <div class="form-row">
                    <div class="col-md-12 mb-10">
                        <p>{{ $t('administrador.componentes.gamificacion.eliminar_accion.modal_restaurar.pregunta') }}</p>
                    </div>
                </div>
                <template v-slot:modal-footer>
                    <div class="btn-group w-100">
                        ...
                        <b-button
                                variant="secondary"
                                size="sm"
                                @click.prevent="$bvModal.hide('undelete-action-modal')"
                        >
                            <i class="fas fa-window-close"></i> {{ $t('no') }}
                        </b-button>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            getActions(page) {
                ...
                axios
                    ...
                    .then(res => {
                        ...
                        this.data.rows = this.actions;
                    })
                    .catch(() => {
                        this.$toastr('error', 'No se han podido cargar las acciones', 'Error al cargar');
                    })
                    .finally(() => {
            ...
            showDeleteActionModal(actionId) {
                this.actionIdDelete = actionId;
                this.$bvModal.show('delete-action-modal');
            },
            showForceDeleteActionModal(actionId) {
                this.actionIdDelete = actionId;
                this.$bvModal.show('force-delete-action-modal');
            },
            showUndeleteActionModal(actionId) {
                this.actionIdUndelete = actionId;
                this.$bvModal.show('undelete-action-modal');
            },
            deleteAction(force) {
                this.loadModalBtn = true;
                if(!force) {
                    axios
                        .delete('/actions/' + this.actionIdDelete, {
                            params: {
                                force_delete: 0
                            }
                        })
                        .then(() => {
                            this.getActions();
                            this.$toastr('success', 'Se ha eliminado temporalmente la acción', 'Eliminación exitosa');
                        })
                        .catch(() => {
                            this.$toastr('error', 'No se ha podido eliminar la acción', 'Eliminación fallida');
                        })
                        .finally(() => {
                            this.actionIdDelete = null;
                            this.loadModalBtn = false;
                            this.$bvModal.hide('delete-action-modal');
                        });
                } else {
                    axios
                        .delete('/actions/' + this.actionIdDelete, {
                        ...
                        .then(() => {
                            this.getActions();
                            this.$toastr('success', 'Se ha eliminado permanentemente la acción', 'Eliminación exitosa');
                        })
                        .catch(() => {
                            this.$toastr('error', 'No se ha podido eliminar la acción', 'Eliminación fallida');
                        })
                        .finally(() => {
                            this.actionIdDelete = null;
                            this.loadModalBtn = false;
                            this.$bvModal.hide('force-delete-action-modal');
                        });
                }
            },
            undeleteAction() {
                this.loadModalBtn = true;
                axios
                    .patch('/actions/' + this.actionIdUndelete + '/undelete')
                    .then(() => {
                        this.getActions();
                        this.$toastr('success', 'Se ha restaurado la acción', 'Restauración exitosa');
                    })
                    .catch(() => {
                        this.$toastr('error', 'No se ha podido restaurar la acción', 'Restauración fallida');
                    })
                    .finally(() => {
                        this.actionIdUndelete = null;
                        this.loadModalBtn = false;
                        this.$bvModal.hide('undelete-action-modal');
                    });
            }
        }
    }
