<template>
    <div class="container my-30" style="min-height: 820px;" :style="mode==='dark'?'background: #080035; color: #fff':''">
        <section class="hk-sec-wrapper">
            <h3 class="hk-sec-title text-center" :class="mode==='dark'?'text-primary':''">{{ $t('administrador.componentes.paginas') }}</h3>
            <div class="row px-10">
                <div class="col-sm">
                    <router-link class="btn btn-sm btn-labeled btn-success float-right" :to="{ path: '/admin/page' }">
                    ...
                    <table class="table table-sm table-hover table-bordered vld-parent" ref="tableContainer">
                        ...
                        <tr v-if="totalRows === 0">
                            <td :colspan="data.columns.length" class="text-center" style="height: 200px;">
                                <p v-if="!loadPages && totalRows === 0">
                                    {{ $t('administrador.table_list.no_entries') }}
                                </p>
                            </td>
                        </tr>
                        <tr v-for="page in data.rows" :key="page.id">
                            <td v-for="column in data.columns" :key="'page-' + page.id + '-' + column.field">
                                <p v-if="!column.customizable">{{ page[column.field] }}</p>
                                <div v-else-if="column.field === 'is_public'" class="text-center">
                                    <div v-if="page.is_public" class="badge badge-success">PÚBLICA</div>
                                ...
                                <div v-else-if="column.field === 'actions'" class="text-center">
                                    ...
                                    <div class="dropdown-menu">
                                        <router-link class="dropdown-item" :to="{ path: '/admin/page/' + page.id }">{{ $t('editar') }}</router-link>
                                        <router-link class="dropdown-item" :to="{ path: '/admin/page/' + page.id + '/delete' }">{{ $t('eliminar') }}</router-link>
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    <div class="row justify-content-end">
                        <ul v-if="totalRows > 0" class="pagination" style="overflow: auto;">
                            ...
                            <li
                                    ...
                                    class="page-item"
                            >
                                <a class="page-link" @click="getPages(i)">{{ i }}</a>
                            </li>
                            ...
                            <li class="page-item" v-bind:class="currentPage + 1 > Math.ceil(totalRows/limit) ? 'disabled' : ''">
                                <a
                                        class="page-link"
                                        @click="getPages(Math.ceil(totalRows/limit))"
            ...
            getPages(page) {
                ...
                axios
                    ...
                    .then(res => {
                        ...
                        this.data.rows = this.pages;
                    })
                    .catch(() => {
                        this.$toastr('error', 'No se han podido cargar las páginas', 'Error al cargar');
                    })
                    .finally(() => {
