<template>
    <div class="container my-30" style="min-height: 820px;" :style="mode==='dark'?'background: #080035; color: #fff':''">
        <section class="hk-sec-wrapper" >
            <h3 class="hk-sec-title text-center" :class="mode==='dark'?'text-primary':''">{{ $t('administrador.componentes.comentarios_denunciados') }}</h3>
                    ...
                    <table class="table table-sm table-hover table-bordered vld-parent" ref="tableContainer">
                        ...
                        <tr v-if="totalRows === 0">
                            <td :colspan="data.columns.length" class="text-center" style="height: 200px;">
                                <p v-if="!loadDenounces && totalRows === 0">
                                    {{ $t('administrador.table_list.no_entries') }}
                                </p>
                            </td>
                        </tr>
                        <tr v-for="denounce in data.rows" :key="denounce.id">
                            <td v-for="column in data.columns" :key="'denounce-' + denounce.id + '-' + column.field">
                                <p v-if="!column.customizable">{{ denounce[column.field] }}</p>
                                <div v-else-if="column.field === 'reason'" class="text-center">
                                    <div class="badge badge-danger">{{ denounce[column.field] }}</div>
                                </div>
                                <div v-else-if="column.field === 'description'" class="align-middle">
                                    <p class="ellipsis">{{ denounce[column.field] }}</p>
                                </div>
                                <div v-else-if="column.field === 'actions'" class="text-center">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" :href="'/admin/denounce/' + denounce.id">Ver</a>
                                        <a class="dropdown-item" :href="'/admin/denounce/' + denounce.id + '/delete'">Eliminar</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    <div class="row justify-content-end">
                        <ul v-if="totalRows > 0" class="pagination" style="overflow: auto;">
                            ...
                            <li class="page-item" v-bind:class="currentPage - 1 < 1 ? 'disabled' : ''">
                                <a
                                        class="page-link"
                                        @click="getDenounces(currentPage - 1)"
                            ...
                            <li
                                    ...
                                    class="page-item"
                            >
                                <a class="page-link" @click="getDenounces(i)">{{ i }}</a>
                            </li>
                            ...
                            <li class="page-item" v-bind:class="currentPage + 1 > Math.ceil(totalRows/limit) ? 'disabled' : ''">
                                <a
                                        class="page-link"
                                        @click="getDenounces(Math.ceil(totalRows/limit))"
            ...
            getDenounces(page) {
                ...
                axios
                    ...
                    .then(res => {
                        ...
                        this.denounces = res.data.results;
                        this.data.rows = this.denounces;
                    })
                    .catch(() => {
                        this.$toastr('error', 'No se han podido cargar las denuncias', 'Error al cargar');
                    })
                    .finally(() => {
                        this.loadDenounces = false;
                        loader.hide();
                    });
            },
            sort(field) {
