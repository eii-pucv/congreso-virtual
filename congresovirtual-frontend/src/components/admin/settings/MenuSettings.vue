    ...
    <div style="min-height: 820px;" :style="mode==='dark'?'background: #080035; color: #fff':''">
        <div class="container mt-25 mb-10">
            <section class="hk-sec-wrapper">
                <h3 class="hk-sec-title text-center" :class="mode==='dark'?'text-primary':''">{{ $t('administrador.componentes.configuracion_menu.titulo') }}</h3>
                <div class="row px-10 mb-20">
                    <div class="col-md-6 offset-md-3">
                        <input type="text" class="form-control" :placeholder="$t('administrador.componentes.configuracion_menu.buscar')" v-model="query">
                    </div>
                </div>
                <div class="row px-10 vld-parent">
                    <div v-if="loadMenuSettings" style="height: 550px;">
                        <Loading
                                :active.sync="loadMenuSettings"
                                :is-full-page="fullPage"
                                :height="128"
                        ></Loading>
                    </div>
                    <div v-if="!loadMenuSettings" class="col">
            ...
            >
                <template v-slot:modal-header>
                    <h6 class="text-white">{{ $t('administrador.componentes.configuracion_menu.modal_edicion.titulo') }}</h6>
                </template>
                <div class="form-row">
                    <div class="col-md-12 mb-10">
                        <label for="text">{{ $t('administrador.componentes.configuracion_menu.modal_edicion.nombre') }}</label>
                        <input id="text" type="text" class="form-control" v-model="nodeInEdit.text">
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-12 mb-10">
                        <label for="url">{{ $t('administrador.componentes.configuracion_menu.modal_edicion.enlace') }}</label>
                        <v-popover>
                            <span class="tooltip-target ml-1"><i class="fas fa-info-circle"></i></span>
                            <template slot="popover">
                                <p>{{ $t('administrador.componentes.configuracion_menu.modal_edicion.popover_enlace') }}</p>
                            </template>
                        </v-popover>
            ...
            >
                <template v-slot:modal-header>
                    <h6 class="text-white">{{ $t('administrador.componentes.configuracion_menu.modal_eliminacion.titulo') }}</h6>
                </template>
                <div class="form-row">
                    <div class="col-md-12 mb-10">
                        <p>{{ $t('administrador.componentes.configuracion_menu.modal_eliminacion.pregunta') }}</p>
                    </div>
                </div>
                <template v-slot:modal-footer>
                    <div class="btn-group w-100">
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                axios
                    .put('/settings/' + this.menuSettings.id, {
                        value: JSON.stringify(this.settingValue)
                    })
                    .then(() => {
                        this.$toastr(
                            'success',
                            this.$t('administrador.componentes.configuracion_menu.mensajes.exito.modificado.generico.cuerpo'),
                            this.$t('administrador.componentes.configuracion_menu.mensajes.exito.modificado.generico.titulo')
                        );
                    })
                    .catch(() => {
                        this.$toastr(
                            'error',
                            this.$t('administrador.componentes.configuracion_menu.mensajes.fallido.modificado.generico.cuerpo'),
                            this.$t('administrador.componentes.configuracion_menu.mensajes.fallido.modificado.generico.titulo')
                        );
                    })
                    .finally(() => {
            ...
            addChildNode(node) {
                if(node.enabled()) {
                    if(node.data.is_root) {
                        node.append({
                            text: this.$t('administrador.componentes.configuracion_menu.nueva_categoria'),
                            children: [],
