...
test('can create a wrapped error', function t(assert) {
    var ServerListenError = WrappedError({
        ...
        type: 'data_server.js.listen-failed',
        requestedPort: null,
        host: null
    });
    ...
    assert.equal(ServerListenError.type, 'data_server.js.listen-failed');
    ...
    assert.equal(JSON.stringify(err2), JSON.stringify({
        ...
        code: 'EADDRINUSE',
        fullType: 'data_server.js.listen-failed~!~error.wrapped-unknown'
    }));
...
test('can wrap real IO errors', function t(assert) {
    ...
    function assertOnError(err, cause, port) {
        ...
        assert.equal(JSON.stringify(err), JSON.stringify({
            ...
            errno: 'EADDRINUSE',
            syscall: 'listen',
            fullType: 'data_server.js.listen-failed~!~' +
